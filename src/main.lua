...
--   eff.tin        — Endorphin's Floating Friend state tracking
--
...
-- ---------------------------------------------------------------------
-- === eff.tin — Endorphin's Floating Friend (EFF) state tracking ===
-- ---------------------------------------------------------------------
...
-- Debug alias: `!eff` prints the current tracked state.
...
-- ---------------------------------------------------------------------
-- === Other-player EFF state tracking ===
-- ---------------------------------------------------------------------
-- Tracks the floater item for each observed groupmate / other player.
-- Mirrors the self-EFF flow but keyed by player name; emits unified
-- shield.up / shield.down events with subject = <PlayerName>.
--
-- State row shape:  eff_others[player] = { item = "<item>" }
--
-- Triggers ported from Quow's *_ON*_EFF set. The " * X is floating
-- around (him|her|it):" status-report variant is deferred because it
-- carries no player name on the line itself — see design §5.3.
